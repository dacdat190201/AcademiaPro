            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        var data = from ur in _dbContext.AspNetUserRoles
                                   join u in _dbContext.AspNetUsers on ur.UserId equals u.Id
                                   join r in _dbContext.AspNetRoles on ur.RoleId equals r.Id
                                   where u.Email == model.Email
                                   select new { r.Name };
                     
                        //string roles = data.Select(t => t.r.Name).FirstOrDefault();
                        var claims = new[]
                        {
                            ...
                            new Claim(ClaimTypes.Role, data.ToString()),
                        };
