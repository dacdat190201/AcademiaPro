...
namespace testapi.Services.MonHocServices
{
    public class MonHocService : IMonHocService
    {
        ...
        public async Task<ApiResult<object>> GetMonhocAllGV(int maChuDe, int? pageNumber, int? pageSize)
        {
            try
            {
                pageNumber = pageNumber ?? 1; // Nếu pageNumber là null, gán giá trị mặc định là 1
                pageSize = pageSize ?? int.MaxValue; // Nếu pageSize là null, gán giá trị mặc định là int.MaxValue để trả về toàn bộ dữ liệu

                var query = (from monhoc in _context.Monhoc
                             join lophoc in _context.Lophoc on monhoc.MaMh equals lophoc.MaMh
                             join giaovien in _context.Giaovien on lophoc.MaGv equals giaovien.MaGv
                                                      where maChuDe == 0 || monhoc.MaChuDe == maChuDe
                             select new ViewMonHocModel()
                             {
                                 MaMH = monhoc.MaMh,
                                 TenMH = monhoc.TenMh,
                                 HinhAnh = monhoc.HinhAnh,
                                 SoLuong = monhoc.SoLuong,
                                 GiaBan = monhoc.GiaBan,
                                 TenGV = giaovien.TenGv,
                                 HinhAnhGv = giaovien.HinhAnhGv,
                                 MaChuDe = monhoc.MaChuDe,
                ...
                return new ApiResult<object>(message: "true", new { total, data });
            }
            catch (Exception ex)
            {
        ...
        //public async Task<ApiResult<object>> GetMonhocAllGV()
        //{
        //    try
        //    {
        //        //var data = await _context.Monhoc.AsNoTracking().ToListAsync();

        //        var data = (from monhoc in _context.Monhoc
        //                    join lophoc in _context.Lophoc on monhoc.MaMh equals lophoc.MaMh
        //                    join giaovien in _context.Giaovien on lophoc.MaGv equals giaovien.MaGv
        //                    select new ViewMonHocModel()
        //                    {
        ...
        public async Task<ApiResult<object>> MonhocDetails(int MaMH)
        {
            try
            {
                ...
                var data = _mapper.Map<MonHocModel>(monhoc);

                return new ApiResult<object>(message: "true", data);               
            }
