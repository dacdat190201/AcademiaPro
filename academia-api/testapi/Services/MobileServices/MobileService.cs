...
namespace testapi.Services.MobileServices
{
    public class MobileService : IMobileService
    {
        ...
        public async Task<ApiResult<object>> GetAllChuDe()
        {
            try
            {
                var data = from c in _context.Chude
                            join m in _context.Monhoc on c.MaChuDe equals m.MaChuDe into monHocGroup
                            from monHoc in monHocGroup.DefaultIfEmpty()
                            join l in _context.Lophoc on monHoc.MaMh equals l.MaMh into lopHocGroup
                            from lopHoc in lopHocGroup.DefaultIfEmpty()
                            join g in _context.Giaovien on lopHoc.MaGv equals g.MaGv into giaoVienGroup
                            from giaoVien in giaoVienGroup.DefaultIfEmpty()
                            group lopHoc by new { c.MaChuDe, c.TenChuDe, c.Images } into grouped
                            select new
                            {
                                MaChuDe = grouped.Key.MaChuDe,
                                TenChuDe = grouped.Key.TenChuDe,
                                Images = grouped.Key.Images,
                                total = grouped.Count(l => l != null)  // Đếm số lớp học khác null
                            };
